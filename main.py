...
async def set_commands():
    commands = [
        ...
        BotCommand(command="top", description="🏆 Топы игроков"),
        BotCommand(command="clantop", description="🏰 Топ кланов"),
        BotCommand(command="limit", description="📊 Мой лимит передачи денег"),
        BotCommand(command="give", description="🎁 Передать деньги (ответом на сообщение)"),
...
if __name__ == '__main__':
    asyncio.run(main())
    empty()from install import empty
import asyncio
import importlib
