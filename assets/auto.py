...
async def automatisation() -> None:
    ...
    scheduler.add_job(autocommands, 'interval', hours=1, timezone=pytz.UTC)          # ✅
    scheduler.add_job(autocommands2, 'interval', minutes=15, timezone=pytz.UTC)      # ✅
    scheduler.add_job(autocommands3, 'interval', minutes=5, timezone=pytz.UTC)       # ✅
    scheduler.add_job(auto_stop, 'interval', seconds=5, timezone=pytz.UTC)           # ✅
    scheduler.add_job(autocommands4, 'cron', hour=0, minute=0, timezone=pytz.UTC)   # ✅
    scheduler.start()